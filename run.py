...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            # model.load_state_dict(torch.load("models/biobert/biobert_model_v2.bin")) 
            # model.load_state_dict(torch.load("output/checkpoint-6239/pytorch_model.bin")) 

            if isinstance(model, BioModelClassify):
                model.loadBCHeader(os.path.join(checkpoint, "biclassify_header.pt"))
                # model.loadBCHeader("./models/biclassify_header.pt")
            model.to(args.device)
