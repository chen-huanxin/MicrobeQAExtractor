...
def compute_predictions_logits(
    ...
    if all_pred_impossible:
        unique_id_to_impossible = {}
        for impossible in all_pred_impossible:
            unique_id_to_impossible[impossible.unique_id] = impossible
    ...
    for example_index, example in enumerate(all_examples):
        ...
        for feature_index, feature in enumerate(features):
            if all_pred_impossible:
                impossible = unique_id_to_impossible[feature.unique_id]
        ...
        _NbestPrediction = collections.namedtuple(  # pylint: disable=invalid-name
            "NbestPrediction", ["text", "start_logit", "end_logit", "char_start"]
        )
        ...
        for pred in prelim_predictions:
            ...
            nbest.append(_NbestPrediction(text=final_text, start_logit=pred.start_logit, end_logit=pred.end_logit, char_start=orig_char_start))
        # if we didn't include the empty option in the n-best, include it
        if version_2_with_negative:
            if "" not in seen_predictions:
                nbest.append(_NbestPrediction(text="", start_logit=null_start_logit, end_logit=null_end_logit, char_start=-1))

            # In very rare edge cases we could only have single null prediction.
            # So we just create a nonce prediction in this case to avoid failure.
            if len(nbest) == 1:
                nbest.insert(0, _NbestPrediction(text="empty", start_logit=0.0, end_logit=0.0, char_start=-1))

        # In very rare edge cases we could have no valid predictions. So we
        # just create a nonce prediction in this case to avoid failure.
        if not nbest:
            nbest.append(_NbestPrediction(text="empty", start_logit=0.0, end_logit=0.0, char_start=-1))
        ...
        for i, entry in enumerate(nbest):
            ...
            output["char_start"] = entry.char_start
