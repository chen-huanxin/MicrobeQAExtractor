...
class IntegrateSquadExample:
    ...
    def __init__(
        ...
        index = 0
        for start_position_character_single in start_position_character:
            if start_position_character_single is not None and not is_impossible:
                self.start_position.append(char_to_word_offset[start_position_character_single])
                self.end_position.append(char_to_word_offset[
                    min(start_position_character_single + len(answer_text[index]) - 1, len(char_to_word_offset) - 1)
                ])
                index += 1
                    ...
                    for qa in qas:
                        ...
                        if is_impossible:
                            ...
                            for answer in answers:
                                ...
                                start_position_character_list.append(answer["answer_start"])

                            example_intergrate = IntegrateSquadExample(
                                qas_id=qas_id,
                                question_text=question_text,
                                context_text=context_text,
                                answer_text=answer_text_list,
                                start_position_character=start_position_character_list,
                                title=title,
                                is_impossible=is_impossible,
                                answers=qa,
                            )
                            ...
                            if augment:
                                if re.search("sensitive to?", question_text):
                                    ...
                                    # new_prefix = "What is the sensitivity profile of "
                                    # new_suffix = " to antimicrobial agents?"
                                    # question_text = new_prefix + content + new_suffix
                                    # example_intergrate = IntegrateSquadExample(
                                    #     qas_id=qas_id,
                                    #     question_text=question_text,
...
class BioSquadExample:
    ...
    def __init__(
        ...
        self.is_impossible = is_impossible
        self.answers = answers
        self.start_pos_char = start_position_character
...
def squad_convert_example_to_features(
    ...
    if is_training and not example.is_impossible:
        ...
        for index in range(len(start_position)):
            actual_text = " ".join(example.doc_tokens[start_position[index] : (end_position[index] + 1)]).lower()
            cleaned_answer_text = " ".join(whitespace_tokenize(example.answer_text[index])).lower()
    ...
    if is_training and not example.is_impossible:
        ...
        for index in range(len(start_position)):
            tok_start_position = orig_to_tok_index[example.start_position[index]]
            if example.end_position[index] < len(example.doc_tokens) - 1:
                tok_end_position = orig_to_tok_index[example.end_position[index] + 1] - 1
            else:
                tok_end_position = len(all_doc_tokens) - 1

            (tok_start_position, tok_end_position) = _improve_answer_span(
                all_doc_tokens, tok_start_position, tok_end_position, tokenizer, example.answer_text[index]
            )
    ...
    for span in spans:
        ...
        if is_training and not span_is_impossible:
            ...
            for index in range(len(tok_start_position_list)):
                if not (tok_start_position_list[index] >= doc_start and tok_end_position_list[index] <= doc_end):
                    out_of_span = True
            ...
            if out_of_span:
                ...
                if tokenizer.padding_side == "left":
                    ...
                    for index in range(len(tok_start_position_list)):
                        start_position = tok_start_position_list[index] - doc_start + doc_offset
                        end_position = tok_end_position_list[index] - doc_start + doc_offset
                        start_position_list.append(start_position)
                        end_position_list.append(end_position)
...
def squad_convert_examples_to_features(
    ...
    if return_dataset == "pt":
        ...
        if not is_training:
            ...
            all_end_positions = []
            for f in features:
                position_num = len(f.start_position)
                extend_start_position = f.start_position + [-1] * (18 - position_num)
                extend_end_position = f.end_position + [-1] * (18 - position_num)
                all_position_nums.append(position_num)
                all_start_positions.append(extend_start_position)
                all_end_positions.append(extend_end_position)
            ...
            all_position_nums = torch.tensor(all_position_nums, dtype=torch.long)
            dataset = TensorDataset(
                all_input_ids,
                all_attention_masks,
                all_token_type_ids,
