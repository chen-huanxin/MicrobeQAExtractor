...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        ...
        # tb_writer = SummaryWriter(log_dir=os.path.join(args.output_dir, "runs", current_time + "_" + socket.gethostname()))
    ...
    optimizer_grouped_parameters = [
        {
            # "params": [p for n, p in model.named_parameters() if p.requires_grad and not any(nd in n for nd in no_decay)], # 添加了`p.requires_grad and`，目的是不对锁住的参数优化
            "params": [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay)], # 添加了`p.requires_grad and`，目的是不对锁住的参数优化
            "weight_decay": args.weight_decay,
        },
        {
            # "params": [p for n, p in model.named_parameters() if p.requires_grad and any(nd in n for nd in no_decay)], # 添加了`p.requires_grad and`，目的是不对锁住的参数优化
            "params": [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay)], # 添加了`p.requires_grad and`，目的是不对锁住的参数优化
            "weight_decay": 0.0
        },
    ]
    ...
    for epoch_no in train_iterator:
        ...
        if args.local_rank in [-1, 0] and args.logging_every_epoch:
            ...
            tb_writer.add_scalar("lr", scheduler.get_lr()[0], epoch_no)
            tb_writer.add_scalar("loss", (tr_loss - logging_loss) / epoch_len, epoch_no) 
            logger.info(f"Epoch-{epoch_no} lr: {scheduler.get_lr()[0]}")
